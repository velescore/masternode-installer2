...
easyrsa_install_ca() {
  ...
  # Generating DH key only if missing, as it takes significant amount of time
  if [ -f "/etc/veles/vpn/keys/dh2048.pem" ] && grep "BEGIN DH PARAMETERS" /etc/veles/vpn/keys/dh2048.pem > /dev/null; then
    echo "Using existing DH key /etc/veles/vpn/keys/dh2048.pem"
  else
    if [ -f "/usr/share/easy-rsa/easyrsa" ]; then        # Easyrsa 3.x
      ./easyrsa gen-dh 2048
